<template>
  ...
  >
    <div v-if="block.action_type !== 'rate_limit'" class="flex flex-row items-center gap-2 w-full">
      <Icon icon="lucide:info" class="w-4 h-4 text-red-500/80" />
      <div class="prose prose-sm max-w-full break-all whitespace-pre-wrap leading-7 text-left text-card-foreground">
        {{ t(block.content || '') }}
      </div>
    </div>
