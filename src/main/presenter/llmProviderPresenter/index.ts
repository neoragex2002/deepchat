        ...
        try {
          ...
          try {
            if (currentToolCalls && currentToolCalls.length > 0) {
              ...
              // Include planned tool calls into this phase response
              try {
                trace.setPlannedToolCalls(
                  currentToolCalls.map((c) => ({ id: c.id, name: c.name, arguments: c.arguments }))
                )
              } catch {}
            }
            trace.writeResponse()
          ...
          // 2) Stash planned tool calls (collect-only)
          if (currentToolCalls.length > 0) {
          ...
          // Collect-only mode: Provider does not execute tools; Thread handles permissions and execution.
          needContinueConversation = false
        } catch (error) {
          if (abortController.signal.aborted) {
