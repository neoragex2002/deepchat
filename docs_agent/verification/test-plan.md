...
**目的：** 本文档提供一套完整的、可执行的测试用例集，旨在系统性地验证 DeepChat Agent 在 Step 0-4 重构后，其核心架构原则（SSoT、阶段性同步、ACE 取消）和关键工作流程（同步屏障、限流、取消、搜索、权限处理）的正确性、鲁棒性和一致性。
...
*   **运行命令：** `pnpm run dev` (开发模式) 或 `pnpm start` (预览模式)。
*   **日志路径：**
...
*   **默认 Agent 策略 (基于代码现状)：**
    * **工具调用限流：** `mode: 'soft_degrade'`, `max: 100` (在一次请求中，允许最多 100 个工具调用被规划)。
    * **权限决策 (以 `shell-server` 默认配置为例)：** `read` (Inspect/Read) 权限为 `AUTOGRANT` (自动批准)；`write` (Write) 权限为 `REQUIRE_USER_PERMISSION` (需手动确认)。
*   **前置操作：**
...
*   **操作步骤：**
    1.  **读文件：** 发送 Prompt：“请在 wsl 下打印 `/mnt/c/dev/deepchat/status.md` 的全文”（要求使用 `wsl bash -lc` 形式）。（由于 `read` 默认自动批准）
    2.  **写文件：** 发送 Prompt：“请在 wsl 的 `/mnt/c/dev/deepchat` 下创建 `temp.md` 文件，内容为 `hello, DeepChat Agent`”（要求使用 `wsl bash -lc` 形式）。在权限请求上点击“允许”。
*   **预期结果：**
    *   **UI：**
        1.  **读文件：** END 后**不应**注入权限块（`read` 默认 `AUTOGRANT`）。工具块直接显示成功，并输出 `status.md` 的内容。R2 输出总结。
        2.  **写文件：** END 后注入一个 `write` 权限块（`write` 默认 `CONFIRM`）。点击允许后，工具块显示成功。R2 输出总结。
    *   **审计日志 (读文件)：** `PERM.plan{planned:1}` → `PERM.inject{pending:0, granted:1, denied:0}` → `EXEC.tool_start` → `EXEC.tool_result{ok:true}` → `EXEC.continue_start`。
    *   **审计日志 (写文件)：** `PERM.plan{planned:1}` → `PERM.inject{pending:1, granted:0, denied:0}` → `PERM.user_action{decision:'grant', permission:'write'}` → `EXEC.tool_start` → `EXEC.tool_result{ok:true}` → `EXEC.continue_start`。
...
#### **3.3. 权限处理场景 (Permission Handling) - `TC-PERM-01` 到 `TC-PERM-03`**
  ...
  * **TC-PERM-01: 用户拒绝**
      ...
      * **预期结果：**
          ...
          * **审计日志：** 包含 `PERM.user_action{tool_call_id: <ID>, decision:'deny', permission:'write'}`。**不应**包含 `EXEC.tool_start`。`EXEC.continue_start` 会被触发（进入 R2 说明）。
          * **IO 日志：** 包含 `type:'tool_exec', ok:false` 且 `error:'permission_denied'` 的条目。

  * **TC-PERM-02: L2 物理层拒绝 (权限不足)**

      * **前置条件：** （模拟）配置 `shell` 工具的 `autoApprove` 包含 `write`（L1 语义层 `AUTO_GRANT`），但通过 L2 `AppArmor` 策略（或文件系统权限）阻止 `wsl` 写入该目录。
      * **操作步骤：** 发送一个需要 `write` 权限的写文件请求。
      * **预期结果：**
          * **UI：** 不会弹出权限请求块（L1 `AUTO_GRANT`）。工具块直接显示执行失败，错误信息应明确指向权限不足，例如 `Permission denied` (来自 `ToolManager` 的 `permission_denied` 错误码) 或 `tool_execution_error` (如果 L2 拒绝未被正确映射)。模型在 R2 阶段解释失败原因。
          * **审计日志：** `PERM.plan{planned:1}` → `PERM.inject{pending:0, granted:1, denied:0}` → `EXEC.tool_start` → `EXEC.tool_result{ok:false}`。
          * **IO 日志：** `tool_exec` 条目 `ok:false`，`response` 信封中 `error` 应为 `permission_denied` 或 `tool_execution_error`，`data` 中包含 L2 返回的权限不足错误信息（如 `exit_code` 非零）。
  ...
  * **TC-PERM-03: 待授权静止态 (Pending State)**
      ...
      * **预期结果：**
          ...
          * **审计日志：** 包含 `PERM.plan{planned:1}` 和 `STREAM.end{final:true}`。**不应**包含 `EXEC.tool_start` 或 `EXEC.continue_start`。

#### **3.4. 限流策略 (Rate Limiting) - `TC-LIMIT-01` 到 `TC-LIMIT-02`**

  * **验证原则：** TP Authority, Phased Sync, SSoT。

  * **目的：** 验证最大工具调用数限制（`max: 100`）策略在 `soft_degrade` 和 `hard_cut` 模式下的行为。

  * **TC-LIMIT-01: `soft_degrade` 模式 (默认)**

      * **操作步骤：** 确保限流模式为 `soft_degrade`，发送一个触发 ≥ 101 个工具调用的请求（例如，一次性创建 101 个文件）。
      * **预期结果：**
          * **UI：** 前 100 个工具调用正常处理并执行。从第 101 个开始，工具块直接显示错误，错误内容为 `tool_call_limit_exceeded`。模型继续作答（R2），并可能对未完成的任务进行说明。可能出现一个全局的“已达上限”提示块。
          * **审计日志：** 包含 `LIMIT.soft_degrade{limit:100, prev:<P>, batch:101}`。`STREAM.end{final:false}` (在限流后)。
          * **IO 日志：** 第 101 个及以后的 `tool_exec` 条目为 `ok:false`，并包含 `error:'tool_call_limit_exceeded'`。

  * **TC-LIMIT-02: `hard_cut` 模式**

      * **前置条件：** 修改代码常量，将限流模式设置为 `hard_cut`。
      * **操作步骤：** 发送一个触发 ≥ 101 个工具调用的请求。
      * **预期结果：**
          * **UI：** UI 显示一个全局错误块，内容为 `common.error.toolCallLimitTurnTerminated`。整个消息的状态变为 `error`。**不应**进入 R2。
          * **审计日志：** 包含 `LIMIT.hard_cut{limit:100, prev:<P>, batch:101}`。消息状态审计为 `ME.emit{...status:'error'}`，且 `STREAM.end{final:true}`。
          * **IO 日志：** 第 101 个及以后的 `tool_exec` 条目为 `ok:false`，并包含 `error:'tool_call_limit_exceeded'`（占位符错误码）或 `tool_call_limit_turn_terminated`（取决于实现细节）。
...
#### **4.1. 取消机制 (ACE) - `TC-CANCEL-01` 到 `TC-CANCEL-03`**
  ...
  * **TC-CANCEL-01: S 流式阶段取消**
      ...
      * **预期结果：**
          ...
          * **审计日志：** 序列为 `CANCEL.start{}` → `CANCEL.done{...}` → `CANCEL.end_fallback_used{...}` (若触发兜底) → `STREAM.end{final:true}`。
          * **IO 日志：** `response.status` 为 `aborted`，`response.userStop` 为 `true`。
  ...
  * **TC-CANCEL-03: X (工具执行) 阶段取消**
      ...
      * **预期结果：**
          * **UI：** 任何未完成的工具（状态为 `loading`）应被更新为“用户已取消”的错误状态。UI 插入一个“用户已取消”错误块。整个消息状态变为 `error`。
          * **审计日志：** 包含 `CANCEL.start{}`。`CANCEL.done` 中可能包含 `placeholdersWritten` 计数。
          * **IO 日志：** 对应的 `tool_exec` 条目（如果已启动）可能被记录为 `ok:false, error:'user_cancelled'`。
...
#### **5.2. `id-only` 配对与 `revision` 护栏 - `TC-ROBUST-02`**
  ...
  * **预期结果：**
      ...
      * **审计日志：** `UI.downgrade_drop` 审计日志可能会记录被丢弃的降级事件。
...
#### **6.3. 权限错误口径**
  ...
  * **L2 权限不足：** 最终工具块显示 `ok:false, error:'permission_denied'`。`ToolManager` 在 L2 拒绝时会设置 `_meta.errorCode='permission_denied'`，该错误码会被 `ThreadPresenter` 捕获并写入最终的 `response` 信封中。

#### **6.4. 限流占位**

  * **`soft_degrade`：** 超限工具块显示 `ok:false, error:'tool_call_limit_exceeded'`。
  * **`hard_cut`：** 超限工具块显示 `ok:false, error:'tool_call_limit_exceeded'`（占位符错误码），且 UI 会额外显示一个内容为 `common.error.toolCallLimitTurnTerminated` 的全局错误块，并且消息状态被设为 `error`。
...
### **7. 故障排查 (Troubleshooting)**
  ...
  * **现象：** 屏障后 (DRAIN/ACK 结束后) 仍有 `STREAM.RESPONSE` 帧渲染到 UI，导致 UI 闪烁或状态混乱。
      ...
      * **预期：** `STREAM.RESPONSE` 在 `BARRIER.gate_on` 之后不应再被转发（通过 `submitWindow` 集合过滤）。
  ...
  * **现象：** 权限路径与口径不一致，例如执行期回退 `pending` 或出现 `permission_required`。
      ...
      * **预期：** L2 权限不足应直接返回 `permission_denied` 错误，无二次确认与回退 `pending`。
