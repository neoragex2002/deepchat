...
**指导原则：** 本文档阐述 DeepChat Agent 为防止资源滥用和无限循环而设计的最大工具调用数限制策略。核心原则是：**默认启用 `soft_degrade` 模式以保证对话连续性和模型对超限的解释能力，不引入干扰性的“配额卡”交互。由 ThreadPresenter (TP) 作为唯一权威进行裁决、持久化和流程控制，Provider 仅负责解析和软提示。**
...
*   **架构职责清晰：** 严格遵循 SSoT (单一真理来源) 和 Phased Sync (阶段性同步) 原则，将限流决策的权威完全保留在后端的协调层（TP），Provider 仅承担解析和软提示的职责。
...
*   **计数口径：**
    ...
    *   **重要：** 计数口径按“计划调用数”计入（与授权或实际执行结论无关），只对“实际进入执行阶段的计划批”累加；若因限流本身（`soft_degrade` 或 `hard_cut`）而未执行，则不累加。凡进入 X 阶段的调用，必须产出结果（不浪费执行）。
    *   **当前上限值：** `MAX_TOOL_CALLS = 100`（定义于 `src/main/presenter/llmProviderPresenter/baseProvider.ts`）。
...
*   **判断时机（关键）：**
    ...
    *   在**同步屏障（Barrier）** 完成后（即 `STREAM.DRAIN/ACK` 机制确保 S 阶段帧已排干）。
    *   在进入**纯 ME 阶段**，准备注入权限块**之前**。
    *   TP 在此刻会汇总本阶段所有 `planned_tool_calls`，进行累计和判断：`current_tool_calls_count + 本批 planned.length > MAX_TOOL_CALLS`。

**流程中的位置概要：**
`S 阶段结束` -> `同步屏障 (DRAIN/ACK)` -> **`限流判断 (TP)`** -> `权限注入/裁决` -> `工具执行` -> `R2`
...
*   **行为 (Behavior):**
    ...
    5.  完成上述 `MESSAGE_EDITED` 提交后，TP 会发送 `STREAM.END { final: false }`，并正常触发 **R2（继续作答）** 流程。
...
*   **Provider (LLMProviderPresenter - Collect-Only):**
    ...
    *   **重要：** Provider **不执行、不授权、不过问配额**。Provider 曾有硬守卫机制，但已按“Provider 软提示、TP 权威裁决”的原则迁移，即 Provider 不再以“达到 MAX 直接 break”的方式中止流。它可能会输出 `maximum_tool_calls_reached` 或 `quota_hint` 供 TP 审计，但**不会中断流**。
...
由于策略不包含用户交互式“配额卡”，UI 的呈现主要通过 `MESSAGE_EDITED` 事件传递的权威消息块来驱动。
...
*   **流式提示 (STREAM Hint):**
    *   系统**可以**在检测到超限时发送一个临时的 STREAM 提示（例如 `maximum_tool_calls_reached`），但此提示**不落库**，且不包含交互按钮。此提示仅在 STREAM 期间显示，`END/ERROR` 后清理。最终状态严格以 `MESSAGE_EDITED` 为准。
...
*   **审计日志记录：** `src/main/logger/logger.ts` (基于 `logging-spec.md`)
